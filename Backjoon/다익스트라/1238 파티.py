...
# version2 양방향으로 해서, 한 번 씩만 다익스트라 사용
...
for _ in range(m):
    s, e, t = map(int, input().split())
    adj_list[s].append((t, e))
    adj_list_reverse[e].append((t, s))

all_time = [0]*(n+1)

def commute_time(node, way):    
    # 각 노드들 까지의 최대시간 dp를 위한 time list
    time = [float('inf')]*(n+1)
    time[node] = 0
    ...
    while heap:
        ...
        if time[now_node] >= now_t:
            for next_t, next_node in way[now_node]:
                total_time = next_t + now_t
...
commute_time(x, adj_list_reverse)
commute_time(x, adj_list)
print(max(all_time))
